...
def get_resnet_features(debug=False, training_type='imgnet', \
...
def get_resnet_features_TEST(debug=False, training_type='imgnet', \
                        image_set_name = 'images_expt1'):
    
    print('making rand data')
    sys.stdout.flush()
    features_raw = np.random.normal(0,1,[10000,262144])
    print('starting pca')
    sys.stdout.flush()
    scores, wts, pre_mean, ev = compute_pca(features_raw)
    print('done')
