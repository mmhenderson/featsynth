...
class Optimizer:
    def __init__(self, model: model.Model, args: Namespace):
        ...
        self.lr = args.lr
        self.jitter_pix = args.jitter_pix
    ...
    def optimize(self) -> torch.Tensor:
        ...
        while step < self.n_steps:
            def closure():
                ...
                # jitter_func = self.get_jitter_func(self.jitter_pix)
            
                # self.model(self.reparam_func(jitter_func(self.opt_image)))
                
                self.model(self.reparam_func(self.apply_random_jitter(self.opt_image, self.jitter_pix)))
    ...
    # @staticmethod
    # def get_jitter_func(jitter_pix):

    #     if jitter_pix==0:
    #         jitt_x = 0; jitt_y = 0;
    #     else:
    #         jitt_x = np.random.choice(np.arange(-jitter_pix, jitter_pix), 1)[0]
    #         jitt_y = np.random.choice(np.arange(-jitter_pix, jitter_pix), 1)[0]

    #     print(jitt_x,jitt_y)

    #     return lambda y: torch.roll(y, shifts=(jitt_x, jitt_y), dims=(2,3))


    @staticmethod
    def apply_random_jitter(opt_image, jitter_pix):
    
        if jitter_pix==0:
            jitt_x = 0; jitt_y = 0;
        else:
            jitt_x = np.random.choice(np.arange(-jitter_pix, jitter_pix+1), 1)[0]
            jitt_y = np.random.choice(np.arange(-jitter_pix, jitter_pix+1), 1)[0]

        print(jitt_x, jitt_y)
        img_jitt = torch.roll(opt_image, shifts=(jitt_x, jitt_y), dims=(2,3))
